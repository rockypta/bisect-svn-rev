...
def check_text_in_command_output(command, text, repo_path=None):
    """
...
def binary_search_revisions(start_rev, end_rev, text, command, repo_path=None):
    """
    ...
    while low <= high:
        ...
        subprocess.run(["svn", "update", "-r", str(mid)], check=True, cwd=repo_path)

        if check_text_in_command_output(command, text, repo_path):
    ...
    if good != -1:
        ...
        if good > start_rev:
            ...
            subprocess.run(["svn", "update", "-r", str(good - 1)], check=True, cwd=repo_path)
            if not check_text_in_command_output(command, text, repo_path):
        ...
        return binary_search_revisions(start_rev, good -1, text, command, repo_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repo-path", help="The path to the SVN repository.")
    ...
    first_rev = binary_search_revisions(args.start_rev, args.end_rev, args.text, args.command, args.repo_path)
